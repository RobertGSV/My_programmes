...
int main(int argc, string argv[])
{
    ...
    for (i = 0; i < plain_len; i++)
    {
        for (j = 0; j < length; j++)
        {
            ...
            if ((isalpha(plain_text[i])) == 0) /// which means non-alpha
            {
    ...
    int r = 0;
    for (r = 0; r < plain_len; r++)
    {
        if (islower(plain_text[r]))
        {
            cypher_array[r] = tolower(cypher_array[r]);

        } // end if
        if (isupper(plain_text[r]))
        {
            cypher_array[r] = toupper(cypher_array[r]);
